...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        # delta_t = F.softplus(
        #     delta.detach().clone() + torch.broadcast_to(delta_bias.detach().clone().unsqueeze(dim=0).unsqueeze(dim=2),
        #                                                 delta.shape))
        # params_for_debug['B_t'] = torch.einsum('ib,jb->ijb', (
        #     delta_t[0], params_for_debug['Sb_x'][0, 0]))  # each B is discretized by the channel's delta
...
class MambaInnerFn(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        out_z = F.linear(rearrange(out_z, "b d l -> b l d"), out_proj_weight, out_proj_bias)
